...
- `gui.py`: Chứa logic render UI bằng Pygame (phiên bản cũ, chạy qua `main.py`).
- `app.py`: **Phiên bản Web UI** dùng Flask + HTML/CSS/JS. Chạy bằng `python app.py`, mở `http://localhost:5000`.
- `main.py`: Entry point cho phiên bản Pygame (chạy `gui.py`).
- `algorithms/`: Thư mục chứa các thuật toán (mỗi thuật toán 1 file).
- `templates/index.html`: HTML layout cho Web UI.
- `static/css/style.css`: CSS styles (Apple-like design).
- `static/js/app.js`: Frontend JavaScript (Canvas rendering, event handling).
...
## 🌐 Hướng dẫn cho Web UI (`app.py`)

Phiên bản Web UI sử dụng Flask (Python) + HTML/CSS/JS. **Thuật toán KHÔNG CẦN THAY ĐỔI** — code trong `algorithms/` hoạt động với cả Pygame và Web.
...
### Kiến trúc Web
- **Backend** (`app.py`): Flask server, API endpoints, chạy thuật toán trong background thread.
- **Frontend** (`templates/index.html` + `static/css/style.css` + `static/js/app.js`): Giao diện HTML/CSS, Canvas rendering, AJAX polling.

### API Endpoints
- `GET /api/state` — Trạng thái grid + thuật toán (Visualize mode)
- `GET /api/race` — Trạng thái race mode
- `GET /api/tree` — Dữ liệu cây khám phá
- `POST /api/action` — Gửi hành động (run, clear, maze, toggle_wall, v.v.)

### Cơ chế Animation
- Thuật toán chạy trong **background thread** (không block web server).
- Frontend **poll** `/api/state` mỗi ~40ms (~25fps).
- Grid được encode thành flat array: `[0,0,1,1,2,3,...]` (0=empty, 1=wall, 2=start, 3=end, 4=visited, 5=frontier, 6=path).
- Frontend vẽ lên `<canvas>` với HiDPI scaling → font sắc nét, không pixel.

### Lưu ý khi sửa Web UI
- CSS dùng **CSS Variables** (`:root { --accent: ... }`) → dễ đổi theme.
- JS dùng **requestAnimationFrame** cho rendering, **setInterval** cho polling.
- Race panels tạo bằng JS DOM, mỗi panel có canvas riêng.
- Tree dùng canvas với pan/zoom (mouse drag + wheel).